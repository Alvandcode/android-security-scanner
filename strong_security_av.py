#!/usr/bin/env python3
"""
Android Security Scanner & AntiVirus - v3.0 (Production-Ready)
...
class AndroidSecurityScanner:
    """Principal-level Android security scanner with hardened implementation."""
    ...
    def __init__(self, args=None):
        ...
        self.quarantine_dir = "/sdcard/.av_quarantine"
        self.whitelist_file = os.path.join(self.log_dir, "whitelist.json")
        ...
        # FIXED: Specific malicious combos only, no generic crypto
    ...
    def _signal_handler(self, signum, frame):
        ...
        print("\n\n⚠️ درخواست توقف دریافت شد. در حال تمیزکاری...")
        self._shutdown_requested = True
        self.log("WARN", "Scan interrupted by user (SIGINT/SIGTERM)")
        # Cleanup any remaining temp directories
        self._cleanup_temp_dirs()
        sys.exit(0)

    def _cleanup_temp_dirs(self):
        """Remove all temporary scan directories."""
        tmp_base = "/tmp"
        if os.path.exists(tmp_base):
            for item in os.listdir(tmp_base):
                if item.startswith("av_scan_"):
                    try:
                        shutil.rmtree(os.path.join(tmp_base, item))
                    except (OSError, PermissionError):
                        pass

    def _load_whitelist(self):
        """Load whitelisted APK hashes."""
    ...
    def _save_whitelist(self):
        """Save whitelisted APK hashes."""
        with open(self.whitelist_file, 'w', encoding='utf-8') as f:
            json.dump({"hashes": list(self.whitelist), "updated": datetime.now(timezone.utc).isoformat()}, f, ensure_ascii=False)
    ...
    def _save_signature_db(self, hashes):
        ...
        with open(sig_file, 'w', encoding='utf-8') as f:
            json.dump({"hashes": hashes, "updated": datetime.now(timezone.utc).isoformat()}, f, ensure_ascii=False)

    def log(self, level, message):
        """Write to audit log with thread-safe append."""
        timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
        log_entry = f"[{timestamp}] [{level}] {message}\n"
        log_file = os.path.join(self.log_dir, f"scan_{datetime.now(timezone.utc).strftime('%Y%m%d')}.log")
        try:
    ...
    def check_security_settings(self):
        ...
        for cmd, name, bad_value in checks:
            out, _, _ = self.run_cmd(cmd, timeout=10)
            if out == bad_value:
    ...
    def fix_security_settings(self):
        ...
        if self.args.get('no_fix'):
            print("🔧 فیکس خودکار غیرفعال است (--no-fix).")
            return
        ...
        for cmd, name, key in fixes:
            out, _, _ = self.run_cmd(f"settings get global {key}", timeout=10)
            if out == "1":
                resp = input(f"   ⚠️ {name} فعال است. آیا می‌خواهید غیرفعال شود؟ (y/n): ").strip().lower()
                if resp == 'y':
                    _, err, code = self.run_cmd(cmd, timeout=10)
                    if code == 0:
    ...
    def scan_processes(self):
        ...
        out, _, _ = self.run_cmd("ps -A -o CMD= 2>/dev/null || ps -o comm=", timeout=10)
        if not out:
            return
    ...
    def scan_network(self):
        """Check active network connections for suspicious ports."""
        print("🌐 بررسی اتصالات شبکه فعال...")
        out, _, code = self.run_cmd("ss -tulpn 2>/dev/null || netstat -tulpn 2>/dev/null || netstat -an", timeout=15)
    ...
    def scan_installed_apps(self):
        ...
        out, _, code = self.run_cmd("pm list packages -f", timeout=30)
        if code != 0 or not out:
            return
    ...
    def check_apk_certificate(self, apk_path, out_dir):
        """Verify APK certificate using keytool with robust parsing."""
        cert_files = []
        ...
        out, err, code = self.run_cmd(f"keytool -printcert -file {shlex.quote(cert_path)}", timeout=15)
        ...
        # Parse certificate info with multiple regex patterns for locale independence
        owner = None
        issuer = None

        # Try multiple patterns for Owner
        for pattern in [r"Owner:\s*(.+)", r"صاحب:\s*(.+)", r"所有者:\s*(.+)"]:
            m = re.search(pattern, out, re.IGNORECASE)
            if m:
                owner = m.group(1).strip()
                break

        # Try multiple patterns for Issuer
        for pattern in [r"Issuer:\s*(.+)", r"صادرکننده:\s*(.+)", r"签发者:\s*(.+)"]:
            m = re.search(pattern, out, re.IGNORECASE)
            if m:
        ...
        # Check self-signed
        if owner and issuer and owner == issuer:
            issues.append("Self-signed certificate")
            self.risk_score -= 5

        # Check expired - try multiple date formats
        valid_until = None
        # Pattern 1: Standard English
        m = re.search(r"until:\s*(.+)", out)
        if m:
            valid_until = m.group(1).strip()

        if valid_until:
            expiry = self._parse_cert_date(valid_until)
            if expiry and expiry < datetime.now(timezone.utc):
                issues.append("Expired certificate")
                self.risk_score -= 8

        # Check common debug keys
        if owner and ("Android Debug" in out or "CN=Android" in out or "O=Android" in out):
    ...
    def hash_file(self, filepath):
        """Calculate SHA256 hash of file with chunked reading."""
    ...
    def decompile_apk(self, apk_path, out_dir):
        """Decompile APK using apktool with safe quoting and timeout."""
        if os.path.exists(out_dir):
            shutil.rmtree(out_dir, ignore_errors=True)
        # FIXED: shlex.quote prevents command injection
        cmd = f'apktool d {shlex.quote(apk_path)} -o {shlex.quote(out_dir)} -f -q'
        _, err, code = self.run_cmd(cmd, timeout=120)
        return code == 0

    def quarantine_file(self, apk_path, reason):
        """Move suspicious APK to quarantine directory."""
        if not self.args.get('quarantine'):
            return False
        try:
            filename = os.path.basename(apk_path)
            quarantine_path = os.path.join(self.quarantine_dir, f"{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}_{filename}")
            ...
            self.log("INFO", f"Quarantined: {filename} -> {quarantine_path}")
            return True
        except Exception as e:
            self.log("ERROR", f"Quarantine failed for {apk_path}: {str(e)}")
            return False
    ...
    def analyze_smali_strings(self, smali_dir):
        """Search suspicious patterns in smali - memory efficient line-by-line with timeout."""
        if not os.path.exists(smali_dir):
            return []

        matches = []
        files_scanned = 0
        max_files = 5000  # Prevent runaway scanning on huge APKs
        start_time = time.time()
        max_scan_time = 60  # Max 60 seconds for smali scan per APK
        ...
        for root, _, files in os.walk(smali_dir):
            ...
            for file in files:
                if files_scanned >= max_files:
                    self.log("WARN", f"Smali scan truncated at {max_files} files")
                    break
                if time.time() - start_time > max_scan_time:
                    self.log("WARN", f"Smali scan timeout after {max_scan_time}s")
                    break
    ...
    def scan_apk_files(self):
        ...
        # FIXED: --quick mode skips deep APK analysis
        if self.args.get('quick'):
            print("   ⚡ حالت سریع: آنالیز عمیق APK رد شد.")
            self.log("INFO", "Quick mode: Skipping deep APK analysis")
            return
        ...
        for apk_path in tqdm(apks, desc="آنالیز APKها", unit="file"):
            ...
            # Whitelist check
            if file_hash and file_hash in self.whitelist:
                print(f"      ✅ فایل در whitelist است، رد شد.")
                continue
            ...
            if file_hash and file_hash in self.known_malware_hashes:
                ...
                if self.args.get('quarantine'):
                    self.quarantine_file(apk_path, "Known malware hash match")
                continue
            ...
            if self.decompile_apk(apk_path, out_dir):
                ...
                if total_risk >= 15:
                    ...
                    if self.args.get('quarantine'):
                        self.quarantine_file(apk_path, f"Risk score: {total_risk}")
    ...
    def generate_html_report(self, report):
        """Generate a rich HTML report with XSS protection via html.escape."""
        ...
        # FIXED: All user-controlled data escaped with html.escape
        def e(text):
...
<head>
    ...
    <style>
        * {{ box-sizing: border-box; }}
        ...
        .code {{ background: #f5f5f5; padding: 2px 6px; border-radius: 4px; font-family: monospace; font-size: 12px; word-break: break-all; }}
    </style>
</head>
<body>
    <div class="container">
        <h1>🔒 گزارش اسکن امنیتی اندروید</h1>
        <p class="subtitle">نسخه {e(self.VERSION)} | اسکن Heuristic پیشرفته</p>

        <div class="score-box {score_class}">
            <span class="score-num">{e(score)}/100</span>
            <span class="score-label">{e(score_text)}</span>
        </div>
...
"""
        for item in report.get('security_settings', []):
            ...
            html_content += f"<tr><td>{e(item['type'])}</td><td><span class='badge {status_badge}'>{e(item['status'])}</span></td><td>{e(item['recommendation'])}</td></tr>"
...
"""
        if report.get('suspicious_processes'):
            ...
            for p in report['suspicious_processes']:
                html_content += f"<li><strong>{e(p['process'])}</strong> — {e(p['threat'])}</li>"
            html_content += "</ul>"
        else:
            html_content += "<p>هیچ فرآیند مشکوکی یافت نشد ✅</p>"
...
"""
        if report.get('network_connections'):
            ...
            for n in report['network_connections']:
                html_content += f"<li><strong>{e(n['connection'])}</strong> — {e(n['threat'])}</li>"
            html_content += "</ul>"
        else:
            html_content += "<p>هیچ اتصال مشکوکی یافت نشد ✅</p>"
...
"""
        if report.get('installed_apps'):
            ...
            for a in report['installed_apps']:
                html_content += f"<li><code class='code'>{e(a['package'])}</code> — {e(a['threat'])}</li>"
            html_content += "</ul>"
        else:
            html_content += "<p>هیچ اپلیکیشن مشکوکی یافت نشد ✅</p>"
...
"""
        for apk in report.get('apk_analysis', []):
            ...
            <div style="margin-bottom:20px; padding:15px; background:#fafafa; border-radius:8px;">
                <h4>{e(apk['file'])} <span class="badge {'b-danger' if '🔴' in apk['status'] else 'b-warning' if '🟡' in apk['status'] else 'b-safe'}">{e(apk['status'])}</span></h4>
                <p><strong>مسیر:</strong> <code class="code">{e(apk.get('path',''))}</code></p>
                <p><strong>SHA256:</strong> <code class="code">{e(apk.get('hash','N/A'))}</code></p>
                <p><strong>مجوزهای خطرناک:</strong> {e(len(apk.get('permissions',[])))}</p>
                <p><strong>رشته‌های مشکوک:</strong> {e(len(apk.get('suspicious_strings',[])))}</p>
                <p><strong>گواهینامه:</strong> <span class="badge {cert_badge}">{e(cert_text)}</span></p>
            </div>
            """
...
"""
        if report.get('fixed_issues'):
            ...
            for fix in report['fixed_issues']:
                html_content += f"<li>✅ {e(fix)}</li>"
            html_content += "</ul>"
        else:
            html_content += "<p>موردی فیکس نشده یا نیازی نبود.</p>"

        html_content += f"""
        </div>

        <div class="footer">
            <p>زمان اسکن: {e(report['timestamp'])}</p>
            <p>Android Security Scanner v{e(self.VERSION)}</p>
        </div>
    </div>
</body>
</html>
"""
    ...
    def run_full_scan(self):
        """Execute full security scan with all modules."""
        print("=" * 50)
        print(f"🚀 Android Security Scanner v{self.VERSION}")
        ...
        if self.quarantined_files:
            print(f"🚫 قرنطینه‌شده: {len(self.quarantined_files)} فایل")
        print("=" * 50)
        self.log("INFO", f"Scan completed. Score: {self.risk_score}/100 - {status}")


def main():
    parser = argparse.ArgumentParser(description="Android Security Scanner v3.0")
    ...
    parser.add_argument("--quarantine", action="store_true", help="Move suspicious APKs to quarantine")
    ...
    if args.watch:
        ...
        try:
            while True:
                ...
                # Reset scanner state for next iteration
                scanner = AndroidSecurityScanner(vars(args))
